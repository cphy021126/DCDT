...
class ViTAdapter(nn.Module):
    def __init__(self, img_size=224, in_channels=1, pretrained_path=None):
        super().__init__()
        ...
        self.vit.patch_embed.proj = nn.Conv2d(in_channels, self.embed_dim, kernel_size=16, stride=16)
        ...
        if pretrained_path and os.path.exists(pretrained_path):
            ...
            print(f"✅ ViT weights loaded from: {pretrained_path}")

        self.vit.pos_embed = nn.Parameter(torch.zeros(1, self.grid_size * self.grid_size + 1, self.embed_dim))
        nn.init.trunc_normal_(self.vit.pos_embed, std=0.02)

    def forward(self, x):
        B = x.shape[0]
        x = self.vit.patch_embed(x)           # [B, C, H, W]
        x = x.flatten(2)  # Final fix: ensure [B, 196, 768]  # Ensure shape [B, N, C]
        # x = x.transpose(0, 1)
        # print("⚠️ 使用 transpose(1, 2) 转置成功")
        # [B, N, C]
        ...
        for blk in self.vit.blocks:
            x = blk(x)
        x = self.vit.norm(x)
        x = x[:, 1:, :].transpose(1, 2).contiguous().view(B, self.embed_dim, self.grid_size, self.grid_size)
        return x

class TransUNetV2(nn.Module):
    def __init__(self, img_size=224, in_channels=1, base_channels=128, num_classes=1, vit_weights=None):
        super().__init__()
        ...
        self.vit = ViTAdapter(img_size=img_size, in_channels=in_channels, pretrained_path=vit_weights)
        self.bridge = nn.Conv2d(768, base_channels * 8, kernel_size=1)
        #self.bridge = nn.Conv2d(768, 1024, kernel_size=1)

        #self.dec1 = DecoderBottleneck(base_channels * 16, base_channels * 2)改
        bridge_channels = 1536  # 你 bridge 的输出
        x3_channels = base_channels * 8  # = 1024
        self.dec1 = DecoderBottleneck(bridge_channels + x3_channels, base_channels * 2)
    ...
    def forward(self, x):
        ...
        x = self.dec4(x)
        out = self.final(x)
        #print("bridge:", bridge.shape, "x3:", x3.shape)
        return out
