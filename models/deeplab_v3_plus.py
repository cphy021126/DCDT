...
class Deeplabv3plus(nn.Module):
    ...
    def forward(self, input):
        ...
        x = self.aspp(x)#[8, 256, 32, 32]
        # 1/4 128 x 128
        low_level_features = self.conv2(low_level_features)
        low_level_features = self.bn2(low_level_features)

        x = F.interpolate(x, low_level_features.size()[2:], mode='bilinear', align_corners=True)#[8, 256, 64, 64]
        x = torch.cat((x, low_level_features), dim=1)#[8, 304, 64, 64]
        x = self.last_conv(x)#N*1*64*64
        x = F.interpolate(x, input.size()[2:], mode='bilinear', align_corners=True)#N*1*256*256
        x = nn.Sigmoid()(x)
        return x
    ...
    def load_backbone(self, model_path):
        if self.backbone_type == 'mobilenetv2':
            ...
            self.backbone_features = load_pretrain_model(self.backbone_features, torch.load('/home/lingeng/TRFE-Net-for-thyroid-nodule-segmentation-main/backbone/resnet50-19c8e357.pth'))
