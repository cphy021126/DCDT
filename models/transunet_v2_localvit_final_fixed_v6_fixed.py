...
class TransUNetV2(nn.Module):
    def __init__(self, img_size=224, in_channels=1, base_channels=128, num_classes=1, vit_weights=None, pretrained_ckpt=None):
        super().__init__()
        ...
        bridge_channels = 1536
        x3_channels = base_channels * 8
        ...
        if pretrained_ckpt and os.path.exists(pretrained_ckpt):
            print(f" Loading teacher weights from: {pretrained_ckpt}")
            state_dict = torch.load(pretrained_ckpt, map_location='cpu')
            for key in list(state_dict.keys()):
                if "conv1" in key:
                    del state_dict[key]
            self.load_state_dict(state_dict, strict=False)
            print("conv1 shape:", self.conv1[0].weight.shape)

        #print(f"🔥 DEBUG: TransUNetV2 initialized with in_channels={in_channels}")
